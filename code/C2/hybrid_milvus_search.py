...
# 之前使用的是Milvus的客户端模式，这里使用服务端模式，更方便1、执行混合搜索（稀疏+密集向量）2、使用高级索引和查询功能 3、适合复杂场景，功能更强大
# 也可以混合使用，使用客户端完成简单操作，使用服务端完成复杂操作（但是不太建议）

COLLECTION_NAME = "dragon_hybrid_demo"
MILVUS_URI = "http://localhost:19530"  # 服务器模式
DATA_PATH = "E:/BJ/Recipe-RAG/data/C2/dragon.json"  # 相对路径
BATCH_SIZE = 50
...
fields = [
    ...
    FieldSchema(name="dense_vector", dtype=DataType.FLOAT_VECTOR, dim=ef.dim["dense"])  # 密集向量
]
...
if not milvus_client.has_collection(COLLECTION_NAME):
    ...
    print("正在为新集合创建索引...")
    sparse_index = {"index_type": "SPARSE_INVERTED_INDEX", "metric_type": "IP"}  # 使用倒排索引;Inner Product（内积）
...
print("\n--- [混合] 稀疏+密集向量搜索结果 ---")
# 创建 RRF 融合器
rerank = RRFRanker(k=60)  # k 值（默认60），用于控制 RRF 算法中的排序平滑程度,k 值越大，排序结果越平滑；越小则高排名结果的权重越突出
